...
x_grid <- seq(-4.5, 3, length = 40)
x_grid <- expand.grid(x_grid, x_grid)
submodel <- keras_model(model$input, model$layers[[3]]$output)
x_grid$h1 <- predict(submodel, as.matrix(x_grid))[, 1]

ggplot(x_grid) +
  geom_tile(aes(Var1, Var2, fill = h1)) +
  geom_point(data = simulation, aes(x.1, x.2, col = y)) +
  scale_fill_distiller(direction = 1) +
  coord_fixed()
...
superheat(h, pretty.order.rows = TRUE, pretty.order.cols = TRUE)
```
