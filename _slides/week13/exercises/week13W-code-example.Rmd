...
The code below uses the same array_tree and map pattern as described in Lecture
13 - 2. Class 7 corresponds to images of horses.
...
h <- predict(activation_model, cifar$x[1:15,,,])

h[1,,,] %>%
  array_tree(margin = 3) %>%
  map( ~ image(as.matrix(.), asp = 1, col = brewer.pal(8, "Blues"), axes = FALSE))
```
