...
ggplot(scores) +
  geom_point(aes(UMAP1, UMAP2), size = 0.9, alpha = 0.5) +
  coord_fixed()
```

```{r}
bind_cols(scores, pbmc %>% select(GNLY))

scores <- scores %>%
  left_join(pbmc)

ggplot(scores) +
  geom_point(aes(UMAP1, UMAP2, col = GNLY), size = 0.9, alpha = 0.5) +
  coord_fixed() +
